    ...
    public class ApplicationProfile : Profile
    {
        public ApplicationProfile()
        {
            ...
            CreateMap<Tenant, TenantModel>()
                ...
                .ForMember(dest => dest.ObservableApartmentIds, opt => opt.MapFrom(src => src.ObservableApartments.Select(a => a.Id)));

            // Landlord → LandlordModel
            CreateMap<Landlord, LandlordModel>()
                .ForMember(dest => dest.Username, opt => opt.MapFrom(src => src.Username.Value))
                .ForMember(dest => dest.ActiveApartmentIds, opt => opt.MapFrom(src => src.ActiveApartments.Select(a => a.Id).ToList()));
