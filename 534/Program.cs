...
class Program
{
    static void Main(string[] args)
    {
        ...
        // Вывод информации по резервуарам
        Console.WriteLine("Выберите резервуар для отображения информации:");
        foreach (var tank in tanks)
        {
            Console.WriteLine($"{tank.Id}: {tank.Name}");
        }
        Console.WriteLine("Введите ID резервуара (или 0 — чтобы показать все):");

        if (int.TryParse(Console.ReadLine(), out int selectedTankId))
        {
            Console.WriteLine("\nРезервуары:");

            foreach (var tank in tanks)
            {
                if (selectedTankId != 0 && tank.Id != selectedTankId)
                    continue;
